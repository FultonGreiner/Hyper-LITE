...
void page_table_setup(void)
{
    ...
    LOG_DEBUG("ttbr0_el2 set to: 0x%x\n\r", (uint64_t)(mmu_table_1.entries));
}
...
{
    uint64_t hcr_el2    = 0x0ULL;
    uint64_t mair_el2   = 0x0ULL;
    uint64_t sctlr_el2  = 0x0ULL;
    uint64_t tcr_el2    = 0x0ULL;
    ...
    LOG_DEBUG("ttbr0_el2: 0x%x\n", ttbr0_el2);
    LOG_DEBUG("mair_el2: 0x%x\n", mair_el2);
    LOG_DEBUG("tcr_el2: 0x%x\n", tcr_el2);
    LOG_DEBUG("hcr_el2: 0x%x\n", hcr_el2);
    LOG_DEBUG("sctlr_el2: 0x%x\n", sctlr_el2);
}
